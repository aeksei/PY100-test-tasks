...
def task_3(n: int) -> List[List[int]]:
    """
...
def task_4(num: int, m: int) -> Tuple[bool, bool]:
    """
    Дано n-значное целое число.

    Определить:
        а) равна ли сумма первых m цифр сумме m последних цифр;
        б) кратна ли 7 сумма его цифр.

    :param num: n-значное целое число
...
def task_5(n: int, m: int) -> Dict[int, int]:
    """
    В некоторой стране используются денежные купюры достоинством в 1, 2, 4, 8, 16, 32, ... 2^m.
    ...
    :param n: Натуральное число n, описывающее имеющуюся сумму денег
    :param m: Натуральное число m, определяющее номинал купюр
